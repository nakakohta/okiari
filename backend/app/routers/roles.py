...
def _referenced_user_count(role_id: str) -> int:
    users = supabase.table("app_users").select("id").eq("role_id", role_id).execute().data or []
    return len(users)


def _ensure_unique_role_code(code: str, exclude_id: str | None = None) -> None:
    existing = (
...
def read_role(role_id: UUID, _: AdminOrLeaderUser) -> dict:
    return require_role(str(role_id))
...
def update_role(role_id: UUID, payload: RoleUpdate, current_user: AdminUser) -> dict:
    role_id_str = str(role_id)
    before = require_role(role_id_str)
...
def delete_role(role_id: UUID, current_user: AdminUser) -> None:
    role_id_str = str(role_id)
    before = require_role(role_id_str)
    ...
    if _referenced_user_count(role_id_str) > 0:
        raise conflict("Role is referenced by app_users")

    supabase.table("app_roles").delete().eq("id", role_id_str).execute()
    write_audit_log(
        actor_user_id=current_user.auth_user_id,
        target_table="app_roles",
        target_id=role_id_str,
